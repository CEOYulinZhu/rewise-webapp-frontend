...
### 🟢 低优先级任务
 ...
 - [✅] 概览页面
  - [✅] 优化概览页面样式
  - [✅] 拆分成组件：合理拆解成组件，方便后续维护和扩展
  - [✅] 调整方案卡片显示：减少视觉干扰，改为折叠式或其它形式（问AI除了折叠式可以解决这类问题，还有其它什么办法）
 - [✅] 详情页面
  - [✅] 拆分组件：顶部卡片，标签页导航
 - [✅] 收藏页面
 ...
 - [✅] 历史记录与收藏页面
  - [✅] 拆分组件：搜索框，筛选区域、时间排序操作按钮
  - [✅] 调整收藏页面中筛选区域有的显示方式，使其与历史页面显示方式保持统一
  - [✅] 调整历史页面中筛选区域按钮的样式
  - [✅] 在历史页面中将筛选区域中的时间排序操作与内容排序独立出来，时间排序按钮使用组件
 - [✅] 鼠标悬浮效果
  - 移除所有页面的鼠标悬浮效果
 - [] 调整概览页面内容显示逻辑：需要先显示每一步处理过程和每一步处理结果（如正在使用小红书搜索工具搜索相关教程等等），再显示处理过程相关内容 + 最终方案卡片（即现有的卡片内容）
 - [] 调整除首页外的背景颜色（没找到合适的背景颜色，目前不用换）
  - [] 修改其它所有页面的背景颜色，与首页背景颜色保持一致，首页的装饰元素不需要
